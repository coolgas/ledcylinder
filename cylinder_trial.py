            ...
            if not PI:
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        sys.exit()
            user_input = input("Enter mode (You have three modes: Letters, RandomColors, ColorWipe): ")
            if user_input.lower() == 'letters':
                letters = input('Enter letters you want to display: ')
                while True:
                    try:
                        scroll_text_display(letters.upper(),random.randrange(0,0xFFFFFF,2),40)
...
def draw_display():
    if PI:
        for x in range(0,20):
            for y in range(0,5):
                strip.setPixelColor(matrix[y*20+x],(Color(int(display[y][x])>>8&0xFF, int(display[y][x])>>16, int(display[y][x])&0xFF)))
        strip.show()
    else:
        for x in range (0,20):
            for y in range(0,5):
